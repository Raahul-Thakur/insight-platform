...
CREATE TABLE IF NOT EXISTS embedding_cache (
  id BIGSERIAL PRIMARY KEY,
  org_id TEXT NOT NULL DEFAULT 'default',
  owner_type TEXT NOT NULL CHECK (owner_type IN ('startup', 'query', 'crawl_page', 'fact')),
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  expires_at TIMESTAMPTZ NOT NULL,
  UNIQUE (org_id, owner_type, owner_id)
);
...
CREATE INDEX IF NOT EXISTS embedding_cache_embedding_idx ON embedding_cache USING ivfflat (embedding vector_cosine_ops);
